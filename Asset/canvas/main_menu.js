...
function drawMainMenu() {
	...
	if (firstTimeOnMainMenu && !muteMusic) {
		//Thing to be done during first initialization
        clearTrigger();
        doMuteMusic();
        
        if(localStorage.getItem(localStorage.getItem("username")+"SavedLevel") == null)
        {
            directLevelSelection(1);
        }
        else
        {
            directLevelSelection(parseFloat(localStorage.getItem(localStorage.getItem("username")+"SavedLevel")));
        }
